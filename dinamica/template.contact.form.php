...
if(isset($_POST['submitted'])) {
		...
		if(trim($_POST['contactName']) === '') {
			$nameError = __("Você esqueceu de digitar o seu nome.", "site5framework");
			$hasError = true;
		} else {
		...
		if(trim($_POST['email']) === '')  {
			$emailError = __("Voc&ecirc; esqueceu de digitar o seu endereço de e-mail.", "site5framework");
			$hasError = true;
		} else if (!eregi("^[A-Z0-9._%-]+@[A-Z0-9._%-]+\.[A-Z]{2,4}$", trim($_POST['email']))) {
			$emailError = __("Você digitou um endereço de e-mail inv&aacute;lido.", "site5framework");
			$hasError = true;
		} else {
		...
		if(trim($_POST['comments']) === '') {
			$commentError = __("Você esqueceu de digitar sua mendagem.", "site5framework");
			$hasError = true;
		} else {
				...
				<div class="container">
								...
								<form id="contactForm" method="POST">
				                    ...
				                    <div class="one-third last">
										<label for="emailinput"><?php _e("E-mail", "site5framework"); ?></label>
										  ...
										  <span class="error" <?php if($emailError != '') echo 'style="display:block;"'; ?>><?php _e("Você esqueceu de digitar o seu endere&ccedil;o de e-mail.", "site5framework");?></span>
				                    </div>
				                    <div class="two-third">
									<label for="Mymessage"><?php _e("Mensagem", "site5framework"); ?></label>
										<textarea cols="20" rows="20" id="Mymessage" name="comments" class="requiredField"><?php if(isset($_POST['comments'])) { if(function_exists('stripslashes')) { echo stripslashes($_POST['comments']); } else { echo $_POST['comments']; } } ?></textarea>
										  <span class="error" <?php if($commentError != '') echo 'style="display:block;"'; ?>><?php _e("Você esqueceu de digitar sua mensagem", "site5framework");?></span>
				                    </div>
				                    <br class="clear" />
					...
					<article id="post-not-found">
						...
						<section class="post_content">
							<p><?php _e("Desculpe, mas o que voc&ecirc; pesquisou não foi encontrado.", "site5framework"); ?></p>
						</section>
						<footer>
						</footer>
					</article>
