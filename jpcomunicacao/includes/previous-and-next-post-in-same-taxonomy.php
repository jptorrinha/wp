...
function be_adjacent_post_link($format, $link, $in_same_cat = false, $excluded_categories = '', $previous = true, $taxonomy = 'category') {
    ...
    {
        if (get_option($var_prefix."portfolio_style") == "filterable")
        {
            $string = '<a href="javascript:show_item_inline(\''. get_permalink($post) . ' #portfolio-item-wrapper\', \'#portfolio-item-wrapper-container\', \'#thumbs\', 600, \'' .$rel. '\')" rel="'.$rel.'">';
        }
        else
        {
            $string = '<a href="javascript:show_item_inline(\''. get_permalink($post) . ' #portfolio-item-wrapper\', \'#portfolio-item-wrapper-container\', \'ul.paginate\', 600, \'' .$rel. '\')" rel="'.$rel.'">';
        }
	}
    else
    {
